...
def main(hisat2Bed,minimap2Bed,gmapBed,baseOutDir,sample,suffix1,db,cnn=0,TPM=0,report=0):
    ...
    tpmDict = tpmRead(TPM) if TPM else 0
    classification_q = {
        'FSM': 0,
        'ISM': 1,
        'NIC': 2,
    ...
    def annotBlue(d):
        Cell = AnnotMoudle.Isoform_anno(id, d[id])
    ...
    def dbRed(Annot):
        ReadsDB.saveReadsinfo(Annot)
        AnnotationDB.saveAnnoDB(Annot)
        ClassificationDB.saveClassification(Annot)
    ...
    with open(bedOut,'w') as bed, open(statOut,'w') as stat:
        ...
        count = 0
        for id in sorted(list(set(list(hisat2)))+list(set(list(minimap2)))+list(set(list(gmap)))):
            count += 1
...
def option(argv):
    ...
    usages = "python3 {} -g gtf -2 short_reads_bed -3 long_reads_bed -o output".format(argv[0])
    p = AP(usage=usages)
...
if __name__ == '__main__':
    ...
    main(args.hisat2, args.minimap2, args.gmap, args.output,args.sample, args.suffix, gtf2db.dict_make(args.gtf), cnn=args.cnn, TPM=args.tpm, report=args.report)