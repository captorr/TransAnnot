    ...
    with open(stat,'r',encoding='utf8') as f:
        for i in f.readlines():
            if not i.strip() or i.startswith('#') or i.startswith('ID'):
                continue
            line = i.strip().split('\t')
            t_id,classification,gene,transcript,chrom = line[0],line[1],line[3],line[4],line[5]
            FLC, TPM = [line[17], line[18]] if mode else [0,0]
            if '|' in chrom:
                chrom = chrom.split('|')[0]
            if t_id not in res:
...
def easyMerge(config,outputDir,mode=''):
    ...
    for i in samples:
        ...
        expRes,bedRes,db = isoformExpParse(exp,mode),bedParse(bed),utils.load_pickle(db)
        dbr.readsMerge(db)
        dba.annotMerge(db)
        dbc.classMerge(db)
        if expRes:
